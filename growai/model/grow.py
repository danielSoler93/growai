...
import numpy as np
from modtox.docking.glide import glide as gl
from modtox.docking.glide import analyse as an
...
#Harcode RDKIT Data
RDKIT="/opt/schrodinger2018-1/mmshare-v4.1/data/RDKit/Data/"
...
def pdb_to_sdf(ligand_pdb, schr="/opt/schrodinger2018-1", output=None):
...
def create_model(hidden_size = 800, dropout=0.2, batch_norm=1, lr=0.001, init_mode="uniform", activation="softplus", max_len=25, vocab_size=13):
  ...
  model.load_weights('../final_subset_cpu.hdf5')
  print(model.summary())
  return model 
